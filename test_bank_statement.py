...
from bank_statement import describe, validate, read_master, write_master
...
class MasterIntegrityTests(unittest.TestCase):
    ...
    def setUp(self):
        temporary_root = Path(__file__).resolve().parent / ".tools" / "bank-tests"
        ...
        mock = patch("bank_statement.extract", return_value=self.result)
        mock.start()
