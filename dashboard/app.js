...
function render() {
  ...
  $('#attention').textContent = state.attention ? `${state.attention} group(s) need attention` : 'One choice per group';
  $('#nav-count').textContent = state.groups.length; $('#folder').textContent = state.folder;
  const groups = visibleGroups(); $('#queue-count').textContent = `${groups.length} groups`;
  ...
  for (const g of groups) {
    ...
    button.setAttribute('aria-pressed', g.id === selected); button.append(node('span', 'group-icon', g.status === 'reviewed' ? '✓' : '▤'));
    const text = node('span', 'group-text'); text.append(node('strong', '', g.files[0].name), node('small', '', `${g.id.replace('group-', '#')} · ${g.files.length} copies${g.status === 'reviewed' ? ' · Done' : ''}`));
...
function renderGroup(group) {
  ...
  if (!group) {$('#empty').textContent = 'No groups match this view.'; return;}
  ...
  $('#group-status').textContent = {pending: 'Awaiting your choice', reviewed: 'Review complete', attention: 'Needs attention'}[group.status];
  $('#group-status').className = `badge ${group.status === 'reviewed' ? 'done' : ''}`;
  $('#evidence-text').textContent = `${group.files.length} copies share the same verified file content. Choose by filename or original location.`;
  $('.evidence strong').textContent = group.errors.length ? 'Verification needs attention.' : 'Same contents. Different copies.';
  if (group.errors.length) $('#evidence-text').textContent = 'Review the file errors below before choosing a copy.';
  ...
  $('#selection-note').textContent = group.status === 'reviewed' ? 'One copy retained. Other copies are recoverable.' : 'Choose a copy to retain. Nothing is permanently deleted.';
  ...
  group.files.forEach((file, i) => {
    ...
    label.append(node('span', '', `COPY ${String(i + 1).padStart(2, '0')}`), node('span', kept ? 'kept-label' : '', kept ? '✓ RETAINED' : file.present ? `${Math.round(file.size / 1024)} KB` : file.available ? 'IN RECOVERY' : 'UNAVAILABLE'));
    top.append(label, node('h3', '', file.name));
    const preview = node('div', 'preview'); preview.append(node('span', 'placeholder', 'Loading preview…'));
    const controls = node('div', 'page-controls'), previous = node('button', '', '←'), count = node('span', '', ''), next = node('button', '', '→'), open = node('a', '', 'Open original ↗');
    ...
    const keep = node('button', 'keep-button', kept ? '✓ Keeping this copy' : file.present ? 'Keep this copy' : 'Undo to choose this copy');
    keep.disabled = busy || !file.present || kept || !!group.errors.length;
    keep.onclick = () => action('/api/keep', {group: group.id, id: file.id}, 'Copy retained. Other copies moved to recovery.');
...
$('#undo').onclick = () => action('/api/undo', {group: selected}, 'Choice undone. All copies are back in the group.');
...
$('#validate').onclick = async () => {
  ...
  try {const result = await api('/api/validate', {}); const panel = $('#validation'); panel.hidden = false; panel.className = `validation ${result.passed ? 'success' : ''}`; panel.textContent = result.passed ? '✓ Exact-duplicate review is complete. You can now run the Codex content-review step. No Codex calls have been started.' : `Still to resolve:\n${result.problems.join('\n')}`; state = await api('/api/state'); token = state.token; render();} catch (error) {toast(error.message);} finally {$('#validate').disabled = false;}
};
