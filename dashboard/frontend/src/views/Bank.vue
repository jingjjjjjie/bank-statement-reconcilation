...
<main>
    <header><div class="breadcrumb">Workspace <span>/</span> Bank statement</div><a class="header-settings" href="/review">Document review</a></header>
    <WorkflowProgress />
    <section class="page-heading"><div><div class="eyebrow">BANK STATEMENT</div><h1>AmBank transactions</h1><p id="bank-note">Loading the extracted bank statement…</p><a id="bank-source-link" href="/source" hidden>Select bank statement PDF</a></div><a id="workbook" class="button dark" href="/api/bank-workbook" download="answer_statement_bank_only.xlsx" hidden>Download bank-only workbook</a></section>
    ...
    <section id="bank-extraction" class="settings-card" hidden>
      ...
      <label class="setting-field"><strong>Statement year</strong><input id="bank-year" type="number" min="1900" max="2100" placeholder="2025"></label>
      <p>Use the year printed on the statement. Extraction creates the bank master for this workspace.</p>
    ...
    <section id="bank-content" hidden>
      ...
      <div class="stage-actions"><button id="check-bank" class="button secondary" type="button">Check bank extraction</button><a id="bank-next" class="button context-next" href="/complete" hidden>Continue to completion</a><p id="bank-step-note" role="status"></p></div>
      <section class="settings-card"><h2>Export styled Excel</h2><p>Uses the saved bank statement style. The export fills bank fields, leaves PARTICULAR and accounting fields blank, and marks supporting-document matches PENDING.</p><div class="bank-export-fields"><label>Company name<input id="export-company" type="text" autocomplete="organization" placeholder="Company Sdn. Bhd."></label></div><button id="export-button" class="button dark" type="button">Export Excel</button><p id="export-error" class="validation" role="alert" hidden></p></section>
