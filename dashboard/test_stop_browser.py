...
from codex_reviewer import ReviewCancelled
...
from vision_workflow import load, prepare
            ...
            try:
                with patch("dashboard.content_review.CodexReviewer", side_effect=lambda *args, **kwargs: next(reviewers)):
                    with sync_playwright() as playwright:
                        ...
                        page.get_by_role("button", name="Run next review batch").click()
                        self.assertTrue(started.wait(timeout=5))
                        expect(page.get_by_role("button", name="Stop review")).to_be_visible()
                        ...
                        expect(page.locator("#content-message")).to_contain_text("stopped")
                        page.get_by_role("button", name="Run next review batch").click()
                        page.wait_for_function("async () => (await (await fetch('/api/content-review')).json()).pairs.length === 1")
                        self.assertEqual(len(load(work)[1]["pairs"]), 1, review.content_error)
                        page.reload()
