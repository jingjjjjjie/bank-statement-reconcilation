...
- **Source folder** opens http://127.0.0.1:8765/source. Choose a supporting-documents folder and a bank-statement PDF separately with the native file picker or a full path. Checking a path is read-only. **Create or open review** explicitly initializes an isolated review under `Development/duplicated/projects/` and moves exact duplicate copies there. **Extract bank statement** uses the selected PDF and supplied year for the active review; an existing master from another statement is never replaced.
