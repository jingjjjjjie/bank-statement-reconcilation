...
class AcceptNextTests(unittest.TestCase):
    def test_save_failure_then_next_and_numbered_groups(self):
        ...
        with sync_playwright() as playwright:
            ...
            expect(page.locator('.editor-footer button')).to_have_count(5)
            expect(page.locator('#undo-discard-document')).to_be_disabled()
            expect(page.locator('#undo-accept-receipts')).to_be_disabled()
            ...
            expect(page.locator('#accept-receipts')).to_have_text('Saving…')
            held.pop().continue_()
            expect(page.locator('#undo-accept-receipts')).to_be_enabled()
            expect(page.locator('#receipt-unit')).to_have_value(keys[0])
            expect(page.locator('#document-review-status')).to_have_attribute('aria-label', 'Extraction accepted')
            page.locator('#undo-accept-receipts').click()
            expect(page.locator('#accept-receipts')).to_be_enabled()
            expect(page.locator('#document-review-status')).to_have_attribute('aria-label', 'Not reviewed')
            expect(page.locator('[data-field="total"]').first).to_have_value('42.00')
            page.reload()
            ...
            expect(page.locator('#accept-receipts')).to_be_enabled()
            expect(page.locator('#add-receipt')).to_be_visible()
            expect(page.locator('#add-receipt')).to_be_disabled()
            expect(page.locator('#discard-document')).to_be_disabled()
            expect(page.locator('#undo-discard-document')).to_be_enabled()
            page.reload()
            ...
            expect(page.locator('#discard-document')).to_be_enabled()
            page.locator('#discard-document').click()
