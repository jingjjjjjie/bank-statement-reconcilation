...
class ReceiptMatchingBrowserTests(unittest.TestCase):
    def test_extraction_review_accept_navigation_and_reload(self):
        ...
        with sync_playwright() as playwright:
            ...
            page.get_by_role("link", name="Review results", exact=True).first.click()
            ...
            expect(page.locator('[data-field="location"]').first).to_be_visible()
            page.locator('[data-field="location"]').first.fill('top, verified')
            page.locator('.piece-details summary').first.click()
            page.get_by_role('button', name='Select piece 2', exact=True).click()
            expect(page.locator('#receipt-pieces fieldset').nth(1)).to_have_class('settings-card active-piece')
            expect(page.locator('[data-field="total"]').nth(1)).to_have_value('15.00')
            ...
            expect(page.locator('[data-field="total"]').first).to_have_value("45.00")
            expect(page.locator("#receipt-pieces fieldset")).to_have_count(2)
            expect(page.locator('[data-field=location]').first).to_have_value('top, verified')
