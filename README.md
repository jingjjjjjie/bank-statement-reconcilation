...
Pass two is implemented in `vision_workflow.py`. Its report distinguishes the same document, revised/conflicting versions, partial overlap, related supporting documents, distinct documents and uncertain cases. Each candidate includes file/page references, matching evidence, differences, confidence and an admin verdict. Confidence is a review aid, not a calibrated probability. `review/supporting-inventory.csv` lists each current source file and extracted page, sheet, or image unit, including nested folders. It includes available invoice numbers, companies, short descriptions, other extracted fields, original source locations, review status, and possible or confirmed matches. Empty extraction fields mean unavailable or pending information. `raw_text` preserves all locally extracted text; image evidence remains in its source file and prepared preview. The CSV is refreshed at every checkpoint and is a readable export of the validated JSON review state. A local score of at least 70% across two matching fields sends a pair straight to original-document comparison, saving a model screening call. All other pairs still receive model screening, so missing fields or a low local score cannot silently exclude a match. The score only selects work; it never confirms a duplicate.
