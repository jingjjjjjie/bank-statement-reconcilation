...
```console
.tools\python\python.exe vision_workflow.py prepare --refresh
```
...
```console
python duplicate_workflow.py check
```

A workspace-local Python runtime is also available:

```console
.tools\python\python.exe duplicate_workflow.py check
```
...
```console
python duplicate_workflow.py organize --root "path/to/supporting" --manifest "new-batch.json"
python duplicate_workflow.py check --manifest "new-batch.json"
```
...
Pass two is implemented in `vision_workflow.py`. Its report distinguishes the same document, revised/conflicting versions, partial overlap, related supporting documents, distinct documents and uncertain cases. Each candidate includes file/page references, matching evidence, differences, confidence and an admin verdict. Confidence is a review aid, not a calibrated probability. `review/supporting-inventory.csv` lists each current source file and extracted page, sheet, or image unit, including nested folders. It includes available invoice numbers, companies, short descriptions, combined totals, other extracted fields, original source locations, review status, and possible or confirmed matches. Empty extraction fields mean unavailable or pending information. `raw_text` preserves all locally extracted text; image evidence remains in its source file and prepared preview. The CSV is refreshed at every checkpoint and is a readable export of the validated JSON review state. `comparison_policy.py` routes a pair straight to original-document comparison only when its combined total and currency match, together with a reference or two other identifying details. Grand totals replace component amounts; otherwise invoice totals or line items are summed once. Unclear or different totals receive normal model screening, so the local rule never excludes a pair or confirms a duplicate.
...
```console
.tools\python\python.exe vision_workflow.py prepare
```
...
```console
.tools\python\python.exe vision_workflow.py run --max-calls 20
```
...
```console
.tools\python\python.exe vision_workflow.py decide --pair "LEFT_HASH:RIGHT_HASH" --verdict keep_both --reviewer "Admin" --reason "Complementary evidence"
```
...
```console
.tools\python\python.exe vision_workflow.py check
```
...
```console
.tools\python\python.exe -m unittest test_duplicate_workflow test_vision_workflow
.tools\python\python.exe test_codex_connection.py
```
