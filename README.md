...
Pass two is implemented in `vision_workflow.py`. Its report distinguishes the same document, revised/conflicting versions, partial overlap, related supporting documents, distinct documents and uncertain cases. Each candidate includes file/page references, matching evidence, differences, confidence and an admin verdict. Confidence is a review aid, not a calibrated probability.
