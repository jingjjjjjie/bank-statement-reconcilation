...
**Current workflow:** Get Started handles exact SHA copies automatically, then opens Documents. Dashboard and CLI runs perform extraction and receipt assembly only; vision duplicate screening/comparison is retired from the normal workflow. Review receipts on `/extraction-review`, then continue to matching. Existing comparison records are retained for audit. The following describes historical comparison tooling.
