...
- A receipt and invoice for the same obligation must not create two independent payable capacities.
...
For each piece record piece_type, payee, description, references, dates, amount, currency, amount_basis, source_locations.
...
At document level retain a short summary, explicitly labelled totals, and readable. Totals are context, never extra pieces or allocation capacity. Do not invent totals by summing ambiguous amounts. Avoid repeating piece descriptions or amounts in prose. Leave conflicting factual fields empty rather than choosing an unsupported value.
