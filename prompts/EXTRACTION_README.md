...
New model output is document context plus pieces. Document-level type and document/piece limitations are omitted; each piece retains its own piece_type. `receipt_assembly.schema.json` adds source-unit coverage. `extraction.legacy.schema.json` is a compatibility validation contract for older records, not the active model output. Python adapters retain old export/routing accessors without asking the model to generate duplicate prose. IDs and approvals are assigned by Python, never the model.
